...
async def upload_voice(
    ...
    **MP3 is the format people actually have.** They record on a phone, they
    ...
    accepted and converted here; the model only ever sees one format.
    ...
    # It also bounds the decode below, which is the part handling a stranger's
    # file.
    if len(audio) > 25 * 1024 * 1024:
        raise HTTPException(400, "that recording is too large")

    # Decoding is the validation. A header check only proves four bytes look
    # right; this proves the model can read what it is about to be handed, and
    # it is the same work as the conversion.
    try:
        wav = tts.to_reference_wav(audio)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc

    folder = store.AVATARS_DIR / avatar.id
    folder.mkdir(parents=True, exist_ok=True)
    (folder / tts.REFERENCE_NAME).write_bytes(wav)
    return {"ok": True, "seconds": tts.reference_seconds(wav)}
